...
motif_count_sum = [0. for _ in range(20)]
motif_count_mean = {}
# save the mean of each motif pattern for each interval
for m in motif_count_interval:
    motif_count_mean[m] = [0. for _ in range(20)]
    for idx in range(len(motif_count_interval[m])):
        motif_count_mean[m][idx] = np.mean(np.array(motif_count_interval[m][idx]))
        motif_count_sum[idx] += np.mean(np.array(motif_count_interval[m][idx]))
