...
# Mark the diffusion edges and the cascade edges
    ...
    for i, r in cascade_set.iterrows():
        if new_nodes_count > 40 or (i == len(cascade_set['source']) - 1):
            ...
            if stop_steep_flag != 1:
                ...
                edge_list = list(
                    set(weighted_edges[cnt_intervals] + weighted_edges[cnt_intervals - 1]))
                edge_list_diffusion = list(set(diff_edges_cur_interval))
                ...
                edge_flag = cascade_graph.new_edge_property("string")  # Arbitrary python object.
                ...
                for (src, tgt) in edge_list_diffusion:
                    if src not in node_cascades_diff_map:
                        node_cascades_diff_map[src] = cnt_nodes
                        v1 = cascade_graph.add_vertex()
                        cascade_vertices[v1] = src
...
if __name__ == '__main__':
    ...
    for idx in range(len(motif_data)):
        try:
            (motif_patterns_inhib_id) = motif_data[idx]
            ...
            motif_patterns_global_list[mid] = [{} for i in range(number_intervals)]
            motif_patterns_global_list_inhib[mid] = [{} for i in range(number_intervals)]
            motif_count_global_list[mid] = [{} for i in range(number_intervals)]
            ...
            #
            #         # if pat_global == False:
            #         # motif_count_global_list[mid][int_prev][m] = []
            #             # motif_patterns_global_list[mid][int_prev][m] = motif_patterns_steep[interval][
            #             #     m]
            #         motif_count_global_list[mid][int_prev][m] = motif_patterns_steep[interval][m]
            #         # else:
    ...
    pickle.dump(motif_count_global_list_inhib, open('motifs_inhib_count_3.pickle', 'wb'))
