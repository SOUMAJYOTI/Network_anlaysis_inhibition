    ...
    for i, r in cascade_set.iterrows():
        if new_nodes_count > 40 or (i == len(cascade_set['source']) - 1):
            ...
            if stop_inhib_flag != 1:
                ...
                for idx_map in range(len(vertex_maps_filtered)):
                    ...
                    while True:
                        ...
                        for idx in map:
                            ...
                            if number_nodes_free > 4 or number_nodes_free == 0:
                                continue
                    ...
                    ind += 1
                    motifs_edges_coverage[motifs_graph_filtered[idx_map]] = len(edges_curr) / len(cascade_graph.edges())
                    motifs_edges_coverage_norm[motifs_graph_filtered[idx_map]] = len(edges_curr) / motifs_count_filtered[ind - 1]

                motif_inhib_network = cascade_graph
                ...
                if stop_inhib_flag == 1:
                    # return both the motif edges and the final network structure
                    return (motifs_edges_coverage, motifs_edges_coverage_norm), mid
...
if __name__ == '__main__':
    ...
    count_invalid = 0
    motifs_coverage = {}
    motifs_coverage_norm = {}
    for idx in range(len(motif_data)):
        try:
            (motifs_coverage_values, motifs_coverage_values_norm), mid = motif_data[idx]
            motifs_coverage[mid] = {}
            motifs_coverage_norm[mid] = {}
            ...
            for m in motifs_coverage_values:
                ...
                motifs_coverage_norm[mid][m] = motifs_coverage_values_norm[m]
    ...
    pickle.dump(motifs_coverage, open('data_coverage_v1/motifs_coverage_edges_min_4'
                                                  + '.pickle', 'wb'))
    pickle.dump(motifs_coverage_norm, open('data_coverage_v1/motifs_coverage_edges_norm_min_4.pickle', 'wb'))
