# this file computes the transition count of each motif in its current interval to its next size in the
# following interval - PRUNE THE SEQUENCE PATTERNS
    ...
    for i, r in cascade_set.iterrows():
        if new_nodes_count > 40 or (i == len(cascade_set['source']) - 1):
            ...
            if cnt_intervals >= 0:
                ...
                # since there is no previous interval for first/steep interval, we continue
                if cnt_intervals == steep_intervals:
                    cnt_intervals += 1
                    new_nodes = []
                    new_nodes_count = 0
                ...
                if stop_inhib_flag == 1:
                    motifs_transition = motifs_transition[:cnt_intervals + 1], mid
                    return (motifs_transition)
...
if __name__ == '__main__':
    ...
    moitf_transition_global_list_steep = {}
    motif_transition_global_list_inhib = {}
    ...
    for idx in range(len(motif_data)):
        try:
            (motif_transition_mid_inhib) = motif_data[idx]
            motif_transition_inhib = motif_transition_mid_inhib[0]
            mid = motif_transition_mid_inhib[1]
            # cnt_interval_steep = len(motif_transition_steep)
            cnt_intervals_inhib = len(motif_transition_inhib)

            # motif_transition_global_list_steep[mid] = [{} for i in range(number_intervals)]
            motif_transition_global_list_inhib [mid]= [{} for i in range(number_intervals)]
            ...
            for int_prev in range(cnt_intervals_inhib):
                ...
                for m1 in motif_transition_inhib[int_prev]:
                    if m1 not in motif_transition_global_list_inhib[mid][interval]:
                        motif_transition_global_list_inhib[mid][interval][m1] = {}
                    for m2 in motif_transition_inhib[int_prev][m1]: # the idea is to see which of the motif transitions are useful
                                                                    # predictive features !!!
                        motif_transition_global_list_inhib[mid][interval][m1][m2] = motif_transition_inhib[int_prev][m1][m2]
    ...
    pickle.dump(motif_transition_global_list_inhib, open('motif_inhib_transition.pickle', 'wb'))
