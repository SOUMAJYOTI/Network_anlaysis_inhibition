            ...
            if stop_steep_flag != 1:
                ...
                for (src, tgt), cur_time in edge_list:
                    ...
                    if cur_time == 1: # diffusion edge
                        ...
                        time_gap = ((int(cur_time - src_first_time) / 60) )/(60*24)
                ...
                if stop_inhib_flag == 1:
                    motif_weights_patterns_cascade_list = motif_weights_patterns_cascade_list[steep_intervals:cnt_intervals + 1], mid
                    return (motif_weights_patterns_cascade_list)
...
if __name__ == '__main__':
    ...
    motif_weights_patterns_global_list = {}
    motif_weights_patterns_global_list_inhib = {}
    ...
    for idx in range(len(motif_data)):
        try:
            (motif_weights_inhib) = motif_data[idx]
            motif_weights_patterns_inhib = motif_weights_inhib[0]
            mid = motif_weights_inhib[1]
            # cnt_interval_steep = len(motif_weights_patterns_steep)
            cnt_intervals_inhib = len(motif_weights_patterns_inhib)

            motif_weights_patterns_global_list_inhib[mid] = [{} for i in range(number_intervals)]
            ...
            for int_prev in range(1, cnt_intervals_inhib+1):
                ...
                for m in motif_weights_patterns_inhib[interval]:
                        motif_weights_patterns_global_list_inhib[mid][int_prev][m] = motif_weights_patterns_inhib[interval][m]
        except:
            count_invalid += 1

    print('Invalid: ', count_invalid)
    pickle.dump(motif_weights_patterns_global_list_inhib, open('motifs_inhib_weights.pickle', 'wb'))
