...
if __name__ == '__main__':
    ...
    entropy_global_acceleration = [{} for interval in range(500)]

    entropy_global_inhib = [{} for interval in range(500)]
    entropy_global_velocity_inhib = [{} for interval in range(500)]
    entropy_global_acceleration_inhib = [{} for interval in range(500)]
        ...
        for i, r in cascade_set.iterrows():
            if new_nodes_count > 40:
                if cnt_intervals >= 0:
                    ...
                    # print(len(list(set(weighted_edges[cnt_intervals] + weighted_edges[cnt_intervals-1]))))
                    # print(len(list(set(weighted_edges[cnt_intervals] + weighted_edges[cnt_intervals-1] + diff_edges_cur_interval))))
                    ...
                    for k, v in top_k_entropy:
                        ...
                        rt_times_node = time_measure_individual[k].return_storage()
                        rt_times_node = list(set(rt_times_node))
                        rt_times_node = sorted(rt_times_node, reverse=True)

                        if len(rt_times_node) <= 1:
                            continue

                        rt_cur_time = rt_times_node[0]
                        rt_last_time = rt_times_node[1]
                        time_diff = int(rt_cur_time - rt_last_time)/60
