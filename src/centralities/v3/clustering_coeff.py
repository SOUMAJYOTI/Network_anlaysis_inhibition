...
if __name__ == '__main__':
        ...
        for i, r in cascade_set.iterrows():
            if new_nodes_count > 40:
                if cnt_intervals >= 0:
                    ...
                    measure_node = nx.clustering(cascade_graph)
                    ...
                    if cnt_prev == 0:
                        measure_velocity_cascade[cnt_intervals] = 0
                    else:
                        measure_velocity_cascade[cnt_intervals] = measure_vel / cnt_prev

                    if stop_steep_flag == 1:
                        for counter_prev_5 in range(1, 100):
                            interval = cnt_intervals + 1 - counter_prev_5
                            try:
                                measure_global[counter_prev_5][mid] = (measure_cascade_sum[interval], number_edges)
                                measure_global_velocity[counter_prev_5][mid] = (measure_velocity_cascade[interval], number_edges)
                            except:
                                break
                        stop_steep_flag = 2
                    ...
                    if stop_inhib_flag == 1:
                        for counter_prev_5 in range(1, 100):
                            interval = cnt_intervals + 1 - counter_prev_5
                            try:
                                measure_global_inhib[counter_prev_5][mid] = (measure_cascade_sum[interval], number_edges)
                                measure_global_velocity_inhib[counter_prev_5][mid] = (measure_velocity_cascade[interval], number_edges)
                            except:
                                break
                        stop_inhib_flag = 2
                        new_nodes_count = 0
    ...
    output_dir = '11_05/cc/cc_change/v3/steep/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    pickle.dump(measure_global_velocity, open(output_dir + 'core_diff' + '.pickle', 'wb'))

    output_dir = '11_05/cc/cc_values/v3/steep/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    pickle.dump(measure_global, open(output_dir + 'core' + '.pickle', 'wb'))

    output_dir = '11_05/cc/cc_change/v3/inhib/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    pickle.dump(measure_global_velocity_inhib, open(output_dir + 'core_diff' + '.pickle', 'wb'))

    output_dir = '11_05/cc/cc_values/v3/inhib/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    pickle.dump(measure_global_inhib, open(output_dir + 'core' + '.pickle', 'wb'))