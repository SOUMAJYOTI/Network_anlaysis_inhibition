...
if __name__ == '__main__':
        ...
        for i, r in cascade_set.iterrows():
            if new_nodes_count > 40:
                if cnt_intervals >= 0:
                    ...
                    measure_node = nx.pagerank(cascade_graph)
    ...
    output_dir = '11_05/pr/pr_change/v3/steep/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    pickle.dump(measure_global_velocity, open(output_dir + 'pr_diff' + '.pickle', 'wb'))

    output_dir = '11_05/pr/pr_values/v3/steep/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    pickle.dump(measure_global, open(output_dir + 'pr' + '.pickle', 'wb'))

    output_dir = '11_05/pr/pr_change/v3/inhib/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    pickle.dump(measure_global_velocity_inhib, open(output_dir + 'pr_diff' + '.pickle', 'wb'))

    output_dir = '11_05/pr/pr_values/v3/inhib/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    pickle.dump(measure_global_inhib, open(output_dir + 'pr' + '.pickle', 'wb'))