...
#                0     ['Model 1-full series', 'Pagerank', 168.279],
...
dpoints_steep = np.array([['Model 1-full series', 'Degree entropy', 140.834],
                    ['Model 1-full series', 'Clustering coeff.', 257.166],
...
def barplot(ax, dpoints):
    ...
    for i, cond in enumerate(conditions):
        ...
        ax.bar(pos, vals, width=width, label=cond,
               color=colors[i])#cm.Accent(float(i) / n))
    ...
    # Add the axis labels
    ax.set_ylabel("Mean Absolute error", size=30, **hfont)
